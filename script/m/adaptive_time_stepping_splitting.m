...
function u1 = lbwe_step_splitting (u0, t, dt, rate, jacrate, mass, tol)

  res = @(t, u, udot) mass * udot - rate{2} (u);
  jac = @(t, u, udot) templatejac (@(t, u, udot) - jacrate{2} (u),
  @(t, u, udot) mass, t, u, udot);
  o = odeset ('Jacobian', jac, 'RelTol', 1e-6, 'InitialStep', dt/2);
...
function u = integrate_adaptive (u0, tspan, tol, mag, rate, jacrate, mass)
  ...
  for ispan = 1 : numel (tspan) - 1
    ...
    while (t < tspan(ispan + 1))
      ...
      unew   = lbwe_step_splitting (u0, t, dt, rate, jacrate, mass);
