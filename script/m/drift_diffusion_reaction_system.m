...
function eqs = drift_diffusion_reaction_system (t, y, ydot, reactions, index, x, N, V0, q, epsilon, Vth, mobility, valence)
  ...
  %% source_term is a matrix NxN_spcies containging the production chemical term for each species by columns in each N grid point
    for ii= 1:N
     source_term(ii, :)= compute_change_rates(rearrange_y(ii,:),reactions,index);
    endfor
