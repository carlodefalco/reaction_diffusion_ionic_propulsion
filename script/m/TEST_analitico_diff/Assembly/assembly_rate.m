...
%===============================================================================
% TEST 1- NO DRIFT-NO CHEM-ONLY DIFFUSIONS
%===============================================================================
   ...
   for k=1:N_species
     A(1+N*(k-1): k*N,1+N*(k-1): k*N) = bim1a_advection_diffusion(x, mobility(k)*Vth(k), 1, 1, 0);
     A_bc(1+(N-2)*(k-1):(N-2)*k,1+(N-2)*(k-1):(N-2)*k) = A(2+N*(k-1):N*k-1,2+N*(k-1):N*k-1);
   endfor

   %chemistry= compute_change_rates2 (y_bc(1:(N-2)*N_species), reactions, index, x(2:end-1)); %chemical source term ((N-2)x N_species)

   F= zeros((N-2)*N_species,1); %compute source term wiht boundary conditions
   for k=1:N_species
     F(1+(N-2)*(k-1):k*(N-2))  = - A(2+N*(k-1):N*k-1, N*(k-1)+1).*bc(k,1) - A(2+N*(k-1):N*k-1,N*k).*bc(k,2);%+Mk(2:end-1, 2:end-1)* chemistry (:,k) ;
   endfor
