...
function dstate_jac = compute_change_rates_jacobian(state, reactions, index)
  ...
  d_Rbi_d_sj=zeros(1,N);
   for [value, key] = the_reaction.reactants
      d_Rfi_d_sj(index.(key)) = the_reaction.rate_coeffs(1);
       d_Rfi_d_sj(index.(key)) *=state(index.(key)) ^ (value-1)* value;% derivative with respect to the index.(key) concentration
