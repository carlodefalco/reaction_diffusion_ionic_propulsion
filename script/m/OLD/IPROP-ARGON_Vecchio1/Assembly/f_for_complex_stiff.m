function eqs = f_for_complex_stiff ( y, reactions, index, x, N, q, epsilon, Vth, mobility, valence, bc, bc_phi)
%===============================================================================
...
%===============================================================================
  ...
  b=q*Mk(2:end-1, 2:end-1)*(valence(1)*n1+valence(2)*n2+valence(3)*n3+valence(4)*n4+valence(5)*n5+valence(6)*n6);

  PHI(2:end-1) = P(2:end-1, 2:end-1) \ (b-P(2:end-1, :) * PHI);
%===============================================================================
...
%===============================================================================
  ...
  %b_no_bc=q*(valence(1)*n1+valence(2)*n2+valence(3)*n3+valence(4)*n4+valence(5)*n5+valence(6)*n6);
...
##  endfor
  ...
  f_1_no_bc   = zeros(79,1);%chemistry (:,1);
  f_2_no_bc   = zeros(79,1);%chemistry (:,2);
  f_3_no_bc   = zeros(79,1);%chemistry (:,3);
  f_4_no_bc   = zeros(79,1);%chemistry (:,4);
  f_5_no_bc   = zeros(79,1);%chemistry (:,5);
  f_6_no_bc   = zeros(79,1);%chemistry (:,6);
  %f_phi_no_bc = b_no_bc;
...
%===============================================================================
  ...
  f_6   = f_6_no_bc - A6(2:end-1,1).*bc(6,1) - A6(2:end-1,end).*bc(6,2);
...
%===============================================================================
  % Mass matrix_type
##  M=zeros((N_species+1)*(N-2),(N_species+1)*(N-2));
...
##
##  % Stiffness matrix
##  A = zeros((N_species+1)*(N-2), (N_species+1)*(N-2));
...
##  A(1+5*(N-2):6*(N-2), 1+5*(N-2):6*(N-2)) = A6(2:end-1, 2:end-1);
##  A(1+6*(N-2):7*(N-2), 1+6*(N-2):7*(N-2)) = P (2:end-1, 2:end-1);
##
##  F = [Mk*f_1; Mk*f_2; Mk*f_3; Mk*f_4; Mk*f_5; Mk*f_6; f_phi];
...
##  eqs=A*u-F;
    ...
    eqs=[ A1(2:end-1,2:end-1)*n1-f_1;
          A2(2:end-1,2:end-1)*n2-f_2;
          A3(2:end-1,2:end-1)*n3-f_3;
          A4(2:end-1,2:end-1)*n4-f_4;
          A5(2:end-1,2:end-1)*n5-f_5;
          A6(2:end-1,2:end-1)*n6-f_6
          ];
