...
mobility(idx.("e"))    = 3.15e-2;%[m^2 / (V*s)]
...
state0(idx.("e"))    = 1.01e+18;%[cm^-3]
state0(idx.("Ar+"))  = 1.0e18;%[cm^-3]
state0(idx.("Ar2+")) = 1.0e16;%[cm^-3]
state0(idx.("Ar*"))  = 1.0e18;%[cm^-3]
...
##mass1=sparse((N_species+1)*(N-2),(N_species+1)*(N-2));
##mass1(1:N_species*(N-2), 1:N_species*(N-2))=M;
mass1=sparse((N_species+1)*N,(N_species+1)*N);
mass1(1:N_species*N, 1:N_species*N)=M;
mass2=M;
mass={mass1; mass2};
rate={@(y,t) assembly_rate_transport(t, y, x, r,idx, valence, mobility,BC_species, BC_phi,N, q, epsilon, Vth, P, Mk);
      @(y,t) assembly_rate_chem(t, y, x, r,idx, valence, mobility,BC_species, BC_phi,N, q, epsilon, Vth, P, Mk)};
jacrate={@(y,t) assembly_jacrate_transport(t, y, x, r,idx, valence, mobility,BC_species, BC_phi,N, q, epsilon, Vth, P, Mk);
         @(y,t) assembly_jacrate_chem(t, y, x, r,idx, valence, mobility,BC_species, BC_phi,N, q, epsilon, Vth, P, Mk)};
keyboard
warning ('off', 'Octave:nearly-singular-matrix')
u = integrate_adaptive (y0, T_vec, tol, mag, rate, jacrate, mass);
keyboard
%===============================================================================
% DATA PROCESSING
%===============================================================================
...
figure()
for ii = 1 :500: numel(t)

 semilogy(x, n1(ii, :),'LineWidth', 1,'r')

 axis ([min(x) max(x) 1e2 2.5e14]);
 str=sprintf('time = %d s', t(ii));
 legend('e')
 title(str)
 hold off
...
for ii = 1 :500: numel(t)
 ...
 semilogy(x, n2(ii, :),'LineWidth', 1,'y')

 axis ([min(x) max(x) 1e24 3e26]);
 str=sprintf('time = %d s', t(ii));
 legend( 'Ar' )
 title(str)
 hold off
...
figure()
semilogy(x, n1(end, :),'LineWidth', 1,'r')
hold on
semilogy(x, n2(end, :)/1e9,'LineWidth', 1,'g')
hold on
semilogy(x, n3(end, :),'LineWidth', 1,'c')
semilogy(x, n4(end, :),'LineWidth', 1,'b')
semilogy(x, n5(end, :),'LineWidth', 1,'m')
semilogy(x,n6(end, :),'LineWidth', 1,'y')
axis ([min(x) max(x) 1e5 2.5e20]);
str=sprintf('time = %d', T_vec(end));
legend('e', 'Ar*1e-9','Ar+', 'Ar*','Ar2+','h_nu', 'Location', 'north')
title(str)
