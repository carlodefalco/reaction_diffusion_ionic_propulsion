...
function jac = compute_change_rates_jacobian2( density, reactions, index,x)

  N_species=numfields(index);
  N=numel(x);
  density=reshape(density,N,N_species);
  jac = sparse(N*N_species, N*N_species);
for ii=1:N

   state=(density(ii,:));
   dstate_jac=sparse(N_species, N_species);
  for the_reaction = reactions(:)'

  d_Rfi_d_sj=zeros(1,N_species);
  d_Rbi_d_sj=zeros(1,N_species);
...
jac(ii:N:N_species*N, ii:N:N_species*N )=dstate_jac;
